...
// homePage is homeHTML with the shared brand favicon and the Kai wave logo (the exact mark the
// dashboard sidebar uses) injected once at init, so the homepage logo matches the app exactly.
var homePage = strings.NewReplacer(
	"<!--FAVICON-->", string(FaviconLink()),
	"<!--MARK-->", sidebarMarkSVG,
).Replace(homeHTML)

// homeHTML is the standalone landing page. Content is kept accurate to what Kaimi does (finds
// SAM.gov opportunities with the user's own key, scores them, drafts proposals for human
// review, never auto-submits, and exports to Word/PDF or the user's Drive). It links the
// privacy policy, which Google's homepage requirement asks for. House style: no em dashes.
...
.nm{font-weight:800;font-size:18px;letter-spacing:-.02em;color:var(--ink);line-height:1;}
.nm small{display:block;font:500 11px/1 -apple-system,Segoe UI,Roboto,sans-serif;color:var(--ink3);letter-spacing:.06em;margin-top:3px;text-transform:uppercase;}
...
<body><div class="wrap">
  <header>
    <div class="brand"><span class="mk"><!--MARK--></span><div class="nm">Kaimi<small>the seeker</small></div></div>
    <nav><a href="/help">Help</a><a href="/privacy">Privacy</a><a href="/entry">Sign in</a></nav>
  </header>

  <section class="hero">
    <div class="eyebrow">Federal business development, automated</div>
    <h1>The agents hunt.<br>You make the calls.</h1>
    <p>Kaimi finds the federal opportunities worth your time, scores them against your capabilities, and drafts compliant proposals, so your team spends its hours deciding and winning instead of searching and formatting.</p>
    <div class="cta">
