...
- 2026-06-24: **Post-checkpoint follow-ups SHIPPED (Malik-approved).** (1) Description resolver WIRED into the live Zone-1 pipeline — RunZone1 resolves the eligible set's noticedesc URL→text before scoring (quota-bounded, non-fatal, Resolved count); cmd/pipeline builds it in live mode; deployed to pilot-kaimi-pipeline job (runs next scheduled hunt). (2) Capability-map rebuild made ASYNC — onboarding save now returns in 0.28s (was ~14.5s); detached 3-min ctx, panic-recovered; deployed to pilot-kaimi-api rev 00012-8kp; verified live (fast save + background rebuild completed, map generated_at fresh). The map-driven SCORING signal stays opt-in/OFF pending a real-profile A/B re-run. TDD + Gemma review (clean) both.
