...
- 2026-06-17: **P2 built + verified — AWAITING MALIK (security checkpoint, PR #92 open, NOT self-merged).** Product-key access gate in `internal/httpapi`: `KAIMI_GATE_MODE=product-key`; `/access?key=` magic link + `/entry` form; `RequireProductKey`/`RequireProductKeyHTML` fail-closed on all app + `/api/v1` routes (exempt: `/healthz`, `/entry`+`/access`, Drive callback); HMAC `__Host-` session carrying only the key id, bounded by key expiry (`SetSessionBounded`), re-validated against the registry per request (instant revoke); Google OAuth only for Drive connect; onboarding CSRF via `DashboardIdentity`. `cmd/api` wires Firestore/in-memory registry + `SESSION_SECRET` + `-dev-seed-key`. TDD (12 gate tests + gate-mode + bounded-session); gate green; security self-verified (no-key→401, expiry, instant revoke, no-bypass, forged/tampered cookie→401, hardened cookie flags) + browser-validated the one-click flow into the dashboard; independent review (Gemma) = CLEAN/EXCELLENT, sole LOW (login-CSRF on `/entry`) recorded as a deliberate single-tenant decision. **Per the P2 directive, the security-sensitive access gate is NOT self-merged — left open for Malik's review/merge.** P3/P4 continue after Malik weighs in.
