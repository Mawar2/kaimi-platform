...
function WPipe({ stageIndex, status }){
  const names = window.KAIMI_STAGE_NAMES;
...
function ReviewCard({ onApprove, onChanges, onOpenDraft }){
  const R = window.KAIMI_REVIEW;
  const writer = window.KAIMI_AGENTS.writer;
  const hue = window.HUE[writer.hue];
  return (
...
function WorkingState({ agentKey, stageIndex }){
  const a = window.KAIMI_AGENTS[agentKey];
  const lines = {
  ...
  return (
    <div className="ws-state">
      ...
      <div>
        ...
        <div className="role">{a.role} agent · {window.KAIMI_STAGE_NAMES[stageIndex]}</div>
      ...
      <div className="ws-head">
        ...
        <div className="ws-id">
          ...
          <div className="ws-meta">
            ...
            <span>{p.status==="human" ? "Paused for your review" : p.status==="done" ? "Ready to submit" : p.status==="submitted" ? "Submitted" : p.status==="queued" ? "Queued — syncs when online" : `${window.KAIMI_STAGE_NAMES[p.stageIndex]} in progress`}</span>
          </div>
        </div>
      </div>
